...
* Context 可以实现跨组件通信，避免了使用 props 逐级传递数据的繁琐。
* 常用于主题颜色、语言和登录验证的全局注入，在 React-redux 中也有应用。
* 可以使用 `React.createContext()` 来创建 Context 对象。当一个 React 组件订阅了 Context 对象时，**这个组件会从组件树中离自身最近的那个 `Provider` 中读取 Context 对象的值。**
* 只有当在组件树中没有匹配到 `Provider` 的值，`defaultValue` 才会生效。
* 每一个 Context 都提供一个 `Provider` 组件，该组件的子组件（消费组件）可以订阅 context 的变化。
* `Provider` 组件有一个 `value` 属性，可以通过该属性将数据传递给订阅了 context 的组件。
* 当 `Provider` 的 `value` 属性发生变化时，所有消费组件都会重新渲染。
* 使用 `React.createContext()` 创建的 context 对象，可以直接赋值给类组件的静态属性 `contextType`。从而可以在类中使用 `this.context` 取得**最近的 Context 的值**。
